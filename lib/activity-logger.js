...
const VERIFIED_CITY_CENTROIDS = Object.freeze({
    'sugar land|US': { lat: 29.61968, lng: -95.63495 },
    'volketswil|CH': { lat: 47.39016, lng: 8.69085 }
});
...
function validateCoordinateResult(coordinates, parts = {}) {
    ...
    const highConfidence = source === 'geocode-city' || source === 'verified-city-centroid';
...
function getVerifiedCityCentroid(parts = {}) {
    const city = cleanValue(parts.city, 100)?.toLowerCase();
    const countryCode = toCountryCode(parts.country);
    if (!city || !countryCode) return null;
    const centroid = VERIFIED_CITY_CENTROIDS[`${city}|${countryCode}`];
    return centroid ? { ...centroid, source: 'verified-city-centroid' } : null;
}
...
async function resolveCoordinates(parts, locationKey, options = {}) {
    ...
    if (!skipRemoteGeocode && normalizedParts.city) {
        ...
        if (!coordinates && (normalizedParts.region || normalizedParts.country)) {
            coordinates = await geocodeByName(normalizedParts.city, countryCode, 'geocode-city');
        }
    }
    ...
    if (!coordinates && normalizedParts.city) {
        coordinates = getVerifiedCityCentroid(normalizedParts);
    }
