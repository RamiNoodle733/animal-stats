...
VoteSchema.statics.getVoteCounts = async function(animalId) {
    const result = await this.aggregate([
        { $match: { animalId: typeof animalId === 'string' ? new mongoose.Types.ObjectId(animalId) : animalId } },
        { $group: {
            _id: null,
    ...
    const counts = result[0] || { upvotes: 0, downvotes: 0 };
    return { upvotes: counts.upvotes, downvotes: counts.downvotes, score: counts.upvotes - counts.downvotes };
};
