...
SiteStatsSchema.statics.recordVisit = async function(ipHash) {
    ...
    // Atomically add the IP to today's uniqueIps set and increment counters.
    // $addToSet prevents duplicate IPs without a read-check-write race.
    ...
    if (!result) {
        // Today's entry doesn't exist yet — push a new daily entry
        const _upsertResult = await this.findOneAndUpdate(
            { key: 'global' },
            {
